...
public class Peon : IDalamudPlugin
{
    ...
    private unsafe void OnDev(string command, string arguments)
    {
        ...
        switch (split[0].ToLowerInvariant())
        {
            ...
            case "opcode":
                if (ProgramHelper.OpcodePrinterEnabled)
                    ProgramHelper.DisableOpcodePrinter();
                else
                    ProgramHelper.AddOpcodePrinter();
                return;
            case "sig":
                ProgramHelper.ScanSig(split[1].Trim());
                return;
            case "staticsig":
                ProgramHelper.ScanStaticSig(split[1].Trim());
                return;
            case "agent":
                if (int.TryParse(split[1].Trim(), out var id))
                    ProgramHelper.PrintAgent(id);
                else
    ...
    private void OnRecipe(string command, string arguments)
    {
        ...
        void PrintRecipe(string pre, Recipe r)
        {
            ...
            foreach (var mat in r.UnkData5.Where(u => u.AmountIngredient > 0))
            {
    ...
    private void OnPeon(string command, string arguments)
    {
